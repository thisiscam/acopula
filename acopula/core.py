...
class Model:
    """A nested-Archimedean copula model traced from a composition function.

    Built by the :func:`defmodel` decorator. Holds the parsed copula tree, the
    flattened parameter array, and exposes the JAX-compatible methods
    ``log_likelihood``, ``cdf``, and ``sample``. Parameters are stored as a
    single ``jax.Array`` so that ``jax.grad`` and ``jax.jit`` operate on the
    whole model uniformly.
    """

    def __init__(self, fn: Callable, default_params: Optional[Any] = None):
        self._fn = fn
        self._graph: Optional[Node] = None
        self._flat_params: Optional[jax.Array] = None

        # If default params provided, initialize immediately
        if default_params is not None:
            self.set_params(default_params)

    def set_params(self, params: Any) -> None:
        """Set parameters and (re)build the computation graph.

        Args:
            params: Arbitrary PyTree of parameters (JAX arrays/scalars/tracers).
        """
        u_placeholder = UPlaceholder()

        # Wrap function to be harvested
        # We need a wrapper that takes 'params' as input because Oryx traces dependencies
        def wrapped_fn(p):
            # We must use 'p' (the tracer version of params) inside the function
            return self._fn(p, u_placeholder)

        # Run function under registration context and harvest parameters
        with RegistrationContext():
            # harvest(tag="params") returns the reaped values for that tag directly.
            # The harvested function expects (plants, *args), so we pass {} for plants.
            graph, harvested = oryx_core.harvest(wrapped_fn, tag="params")({}, params)
            param_dict = harvested
        ...
        # Validate marginal leaves (required)
        leaves_in_order = _collect_leaves_in_order(graph)
        obs_shape = _infer_obs_shape_and_validate_unique(leaves_in_order)
        if obs_shape is None:
            raise ValueError(
                "Model must specify marginal(...) leaves (no backwards-compatible flat-U output)."
            )

        # Build the global flattened params vector by concatenating per-instance flat vectors
        # in increasing `start` order (we used start as the sow name).
        sorted_items = sorted(param_dict.items(), key=lambda kv: int(kv[0]))
        flat_parts = [jnp.asarray(v).reshape((-1,)) for _, v in sorted_items]
        self._flat_params = jnp.concatenate(flat_parts) if flat_parts else jnp.array([])

        self._graph = graph

    # ----------- Structure access -----------
    def graph(self) -> Node:
        """Return the structural graph (Node) of the model."""
        if self._graph is None:
            raise RuntimeError(
                "Model graph has not been built yet. "
                "Call set_params() or sample() with parameters first to define the structure."
            )
        return self._graph

    def as_networkx(
        self, include_leaves: bool = True, annotations: Optional[Dict] = None
    ):
        """Build and return a NetworkX DiGraph for the model structure."""
        from .visualize import to_networkx

        node = self.graph()
        return to_networkx(node, include_leaves=include_leaves, annotations=annotations)
    ...
    def visualize(
        ...
        *,
        ...
        """Draw the model structure using NetworkX and matplotlib.

        Args:
            include_leaves: include variable leaves in the graph
            layout: 'hierarchical' (graphviz if available) or 'spring'
            with_labels: annotate nodes with labels
            graphviz_prog: Graphviz program for hierarchical layout
            annotations: Optional dict mapping Node -> annotation string
            ax: optional matplotlib axes
        Returns:
            (fig, ax, graph): matplotlib figure/axes and the NetworkX graph
        """
        from .visualize import draw_networkx

        G = self.as_networkx(include_leaves=include_leaves, annotations=annotations)
        fig, ax = draw_networkx(
            G,
            layout=layout,
    ...
    def sample(
        ...
        *,
        ...
        """Generate n samples from the copula model.

        Args:
            key: JAX PRNG key.
            n: Number of samples to generate.
            params: Optional parameters (triggers graph rebuild if provided).
            max_cdf_x: Upper bound for frailty bisection (Marshall-Olkin only).
            method: Sampling algorithm — "marshall_olkin" (default) or "rosenblatt".
                    Rosenblatt uses the conditional CDF (Rosenblatt transform)
                    and works for both nested and non-nested copulas.

        Returns:
            Array of shape (n, *obs_shape) with sampled observations.
        """
        if params is not None:
            self.set_params(params)

        if self._graph is None:
            raise RuntimeError("Parameters must be provided to build the model graph.")
        ...
        samples = jax.vmap(sample_one)(sample_keys)
        return samples
    ...
    def log_likelihood(
        ...
        *,
        ...
        force_integral_method: bool = False,
        return_nesting_penalty: bool = False,
        survival: bool = False,
        ...
        # Validate observation shape against the model marginals
        leaves_in_order = _collect_leaves_in_order(self._graph)
        obs_shape = _infer_obs_shape_and_validate_unique(leaves_in_order)
        if obs_shape is None:
            raise ValueError(
                "log_likelihood requires marginal(...) leaves to map observations."
            )
        ...
        if obs.shape == obs_shape:
            is_batch = False
        elif obs.ndim == len(obs_shape) + 1 and obs.shape[1:] == obs_shape:
            is_batch = True
        else:
            raise ValueError(
                f"Observed data shape {obs.shape} does not match model requirement {obs_shape} "
                f"(or batch of {obs_shape})."
            )

        # Dynamic censoring requires the Bell path.
        if censored_mask is not None:
            if method not in ("auto", "bell"):
                raise ValueError(
                    f"censored_mask requires method='bell' or 'auto' "
                    f"(got method={method!r})."
                )
            if force_integral_method:
                raise ValueError(
                    "censored_mask is incompatible with force_integral_method=True; "
                    "dynamic censoring is only supported by the Bell polynomial path."
                )
            mask_arr = jnp.asarray(censored_mask)
            # Accept either a single (d,) mask or a per-obs batch.
            if mask_arr.shape == obs_shape:
                mask_per_obs = None
                mask_static = mask_arr
            elif is_batch and mask_arr.shape == obs.shape:
                mask_per_obs = mask_arr
                mask_static = None
            else:
                raise ValueError(
                    f"censored_mask shape {mask_arr.shape} does not match "
                    f"observation shape {obs.shape}; expected {obs_shape} "
                    f"or {obs.shape}."
                )
        else:
            mask_per_obs = None
        ...
        def log_lik_single(x, m=mask_static):
            depth = _depth(self._graph)
            _nesting_pen = jnp.array(0.0)
            if method == "bell" or (method == "auto" and depth > 1) or m is not None:
                from .bell import _log_likelihood_bell
                _, log_val, _nesting_pen = _log_likelihood_bell(
                    self._graph, x, self._flat_params,
                    censored_mask=m,
                    survival=survival,
                )
            elif depth <= 1 and not force_integral_method and method != "integral":
            ...
            if return_nesting_penalty:
                return log_val, _nesting_pen
            return log_val

        if is_batch:
            if mask_per_obs is not None:
                return jax.vmap(log_lik_single)(obs, mask_per_obs)
            return jax.vmap(log_lik_single)(obs)
        return log_lik_single(obs)
    ...
    def make_ll_fn(
        ...
        *,
        ...
        """Return a JIT-friendly log-likelihood function.

        Performs all Python-level dispatch (method selection, graph building)
        once at setup time.  The returned function has signature::

            ll_fn(obs: jax.Array, flat_params: jax.Array) -> jax.Array

        where ``obs`` is a single observation vector and ``flat_params`` is
        the flat parameter vector matching the graph layout.  The function
        is pure JAX and safe to pass to ``jax.jit``, ``jax.vmap``,
        ``jax.value_and_grad``, etc.

        When ``with_censored_mask=True`` the returned function takes an
        extra ``censored_mask`` argument::

            ll_fn(obs, flat_params, censored_mask) -> jax.Array

        so callers can vmap per-observation masks for dynamic censoring.
        This forces the Bell polynomial path, which is the only method
        that supports dynamic censoring.
        """
        if params is not None:
            self.set_params(params)
        if self._graph is None:
...
def defmodel(fn: Callable) -> Model:
    """Decorator that builds a Model from a user-defined nested copula composition.
